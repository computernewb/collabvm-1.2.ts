...
export default class GeoIPDownloader {
	...
	private async ensureDirectoryExists(): Promise<void> {
		...
		try {
			...
			if (error.code === 'ENOTDIR') {
				...
				this.logger.error({ err: error }, 'Failed to access GeoIP directory');
				process.exit(1);
			}
	...
	async getGeoIPReader(): Promise<ReaderModel> {
		...
		try {
			...
			this.logger.info({ path: dbpath }, 'Loading cached GeoIP database');
		} catch (ex) {
			var error = ex as NodeJS.ErrnoException;
			if (error.code === 'ENOENT') {
				await this.downloadLatestDatabase();
			} else {
				this.logger.error({ err: error }, 'Failed to access GeoIP database');
				process.exit(1);
			}
		}
	...
	async downloadLatestDatabase(): Promise<void> {
		...
		this.logger.info({ filename }, 'Downloading latest GeoIP database');
		...
		await file.close();
		this.logger.info({ filename }, 'Finished downloading latest GeoIP database');
		this.logger.info({ filename }, 'Extracting GeoIP database');
		// yeah whatever
