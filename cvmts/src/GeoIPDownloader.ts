...
export default class GeoIPDownloader {
	...
	private async ensureDirectoryExists(): Promise<void> {
		...
		try {
			...
			if (error.code === 'ENOTDIR') {
				...
				this.logger.error(`Failed to access GeoIP directory: ${error.message}`);
				process.exit(1);
			}
			this.logger.info(`Creating GeoIP directory: ${this.directory}`);
	...
	async getGeoIPReader(): Promise<ReaderModel> {
		...
		try {
			...
			this.logger.info(`Loading cached GeoIP database: ${dbpath}`);
		} catch (ex) {
			var error = ex as NodeJS.ErrnoException;
			if (error.code === 'ENOENT') {
				await this.downloadLatestDatabase();
			} else {
				this.logger.error(`Failed to access GeoIP database: ${error.message}`);
				process.exit(1);
			}
		}
	...
	async downloadLatestDatabase(): Promise<void> {
		...
		this.logger.info(`Downloading latest GeoIP database: ${filename}`);
		...
		await file.close();
		this.logger.info(`Finished downloading latest GeoIP database: ${filename}`);
		this.logger.info(`Extracting GeoIP database: ${filename}`);
		// yeah whatever
